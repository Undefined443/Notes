...
*临时量：编译器为了暂存表达式的求值结果临时创建的一个未命名对象*
...
对于有序容器(没有 `unordered_` 前缀的)，标准库使用关键字类型的 `<` 运算符来比较两个关键字。我们也可以提供自己定义的操作(函数)来代替关键字上的 `<` 运算符。所提供的操作必须在关键字类型上定义一个严格弱序(小于)。如果两个元素都不"严格弱序"于对方，则它俩相等。
...
```cpp
make_shared<T>(args);     //返回一个 shared_ptr，指向一个由 args 初始化(构造)的T类型对象
