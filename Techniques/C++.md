...
>
> 第二种 new 叫做定位 new (placement new) ，它通过 new 关键字后面的括号接受一些额外的参数。
...
if (pipe(fd) == -1)
  return -1;  // 创建管道，fd[0] 被设置为读端，fd[1] 被设置为写端。
pid_t pid = fork();  // 创建子进程
// 父进程
if (pid > 0) {
  close(fd[0]);  // 关闭读端
  string str("message");
  write(fd[1], str.c_str(), str.size());  // 写管道
}
// 子进程
else if (pid == 0) {
  close(fd[1]);  // 关闭写端
...
```c
wait(int *status); // 暂停当前进程的执行，直到有信号来到或子进程结束。子进程的结束状态由 status 参数返回。
waitpid(pid_t pid, int *status, int options); // 等待指定的进程。options 可以设为 0 。
// 其中，pid 可以有如下取值：
// pid > 0：等待进程识别码为 pid 的子进程
