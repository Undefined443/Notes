...
inline void merge(int v1, int v2) {
  parent[findParent(v2)] = findParent(v1); //把 v2 所属集合归并到 v1
}
```
...
void TopologicalSort(ALGraph G){
  int indegree[100]; //记录各顶点入度的数组
  FindInDegree(G,indegree); //统计各顶点的入度
  stack<Node> nodeStk;
  for (int i = 0; i < n; ++i) { //将入度为 0 的顶点入栈
    if (!indegree[i]) {
      nodeStk.push(i);
    }
  }
  int count = 0;
  while (!nodeStk.empty()) { //当栈为空，说明排序完成
    int index = nodeStk.top();
    nodeStk.pop();
    //弹栈，并记录栈中保存的顶点所在邻接表数组中的位置
    cout << index + 1;
    ++count;
    //将与 index 相连的顶点的入度 -1
    for (Node *p = G.vertices[index].firstarc; p; p = p->nextarc) {
...
1. 建立顶点的最早发生时间、最晚发生时间表。
2. 根据顶点的最早发生时间表建立边的最早开始时间表（边的最早开始时间就是起点的最早发生时间）。
3. 根据顶点的最晚发生时间表建立边的最晚开始时间表（边的最晚开始时间就是终点的最晚发生时间减去边的长度）。
4. 比较边的最早开始时间表和最晚开始时间表，值相同的边即为关键活动。
...
将网中顶点分成 $S$, $U$ 两组，$S$ 为已求出最短路径的顶点集合，初始时 $S$ 中只有一个源点。$U$ 包含其他顶点，且 $U$ 中顶点的距离为源点邻接该顶点的距离。以后每次都将路径最短的顶点加入到 $S$ 中，并刷新最短路径，直到所有顶点都加入到 $S$ 中为止。
...
> [知乎：弗洛伊德算法](https://zhuanlan.zhihu.com/p/139112162)中给出的邻接矩阵的对角元应该是无穷大才对
...
& {\rm ASL}_{失败}=\frac{1}{m}\sum每个H(key)根据冲突处理方法比较几位找到NULL
\end{aligned}
$$
...
没看懂。[C 语言中文网：希尔排序](http://c.biancheng.net/view/vip_3443.html)
...
堆排序的基本思想是：将无序序列构造成一个大顶堆，此时整个序列的最大值就是堆顶，将其与堆尾交换，并将堆尾摘除，放在数组末尾。然后将剩余 n-1 个元素重新构造成一个大顶堆，将堆顶与堆尾。如此反复执行，便能得到一个有序序列了。
...
void max_heapify(int arr[], int beg, int end) {
  int parent = beg; //父结点索引
  int child = parent * 2 + 1; //子结点索引
  while (child < end) {
    if (child + 1 < end && arr[child] < arr[child + 1]) //选择最大的子结点
      ++child;
    if (arr[parent] > arr[child]) //若父结点大于子结点，则调整完毕
      return;
    else {
