...
首先构造一个邻接矩阵，这个邻接矩阵同时表示网中任意两顶点在不借助任何中转顶点的情况下的最短路径。

接下来，我们让顶点 v~0~ 作为中转站，计算网中各顶点间的最短路径。即对矩阵中每一项，令 `d[i][j] = min(d[i][j], d[i][0] + d[0][j])`。

此时的矩阵表示网中任意两顶点在可以借助 v~0~ 中转的情况下的最短路径。

接下来让顶点 v~1~ 作为中转站，计算网中各顶点间的最短路径。此时的矩阵表示网中任意两顶点在可以借助 v~1~, v~2~ 中转的情况下的最短路径。

如此重复，直到所有顶点都可作为中转结点为止。此时的矩阵就表示网中任意两点间的最短路径了。

```cpp
void Floyd() {
  for (int i = 0; i < n; ++i) {
    Graph[i][i] = 0; //这里对角元应该为 0
  }
  for (int k = 0; k < n; ++k) { //以 vk 为中转顶点
    for (int i = 0; i < n; ++i) {
      for (int j = 0; j < n; ++j) {
